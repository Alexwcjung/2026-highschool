...
with tab_activity:
    ...
    # 활동 1. Key Words 뜻 쓰기
    # -----------------------------------------------------
    st.subheader("활동 1. Key Words 뜻 쓰기")
    st.caption("영어 핵심 단어를 보고 한국어 뜻을 적으세요. 작은 카드 없이 단순한 목록 형태로 제시합니다.")

    activity1_prefix = f"{category}_{topic_name}_activity1_"
    if st.button("🔄 활동 1 다시 풀기", key=f"reset_activity1_{category}_{topic_name}", use_container_width=True):
    ...
    # 활동 2. 지문 해석 빈칸 쓰기
    # -----------------------------------------------------
    st.subheader("활동 2. 지문 해석 빈칸 쓰기")
    st.caption("지문을 그대로 읽고, 해석의 빈칸에 Key Words의 한국어 뜻을 적으세요. 문장도 단순한 줄글 형태로 제시합니다.")

    activity2_prefix = f"{category}_{topic_name}_activity2_"
    if st.button("🔄 활동 2 다시 풀기", key=f"reset_activity2_{category}_{topic_name}", use_container_width=True):
    ...
    for line_no, (speaker, eng, kor) in enumerate(dialogue, start=1):
        ...
        st.markdown(f"**{line_no}. {speaker}: {eng}**")
        st.markdown(f"🇰🇷 {blank_kor}")
        ...
        if matched_words:
            for j, (word, correct_meaning) in enumerate(matched_words, start=1):
                ...
                b1, b2, b3 = st.columns([2, 3, 1.3])
                with b1:
                    st.write(f"{line_no}-{j}. **{word}**")
                with b2:
                ...
                if status_key in st.session_state:
                    if st.session_state[status_key]:
                        st.success(f"{line_no}-{j}번 정답")
                    else:
                        st.error(f"{line_no}-{j}번 정답: {correct_meaning}")
        else:
            st.caption("이 문장에는 빈칸으로 만들 핵심 단어가 없습니다.")
    ...
    if st.button("🔄 활동 3 다시 풀기", key=f"reset_activity3_{category}_{topic_name}", use_container_width=True):
    ...
    # -----------------------------------------------------
    st.subheader("활동 4. Reflection Writing")
