...
<div class="activity-box">
    ...
    Ronaldo를 통해 본인이 배울 점을 적어 봅시다.<br>
...
""", unsafe_allow_html=True)
    ...
    reflection = st.text_area(
        ...
        placeholder=(
            "예: I learned that I should believe in myself and never give up.\n"
    ...
    if st.button("💬 피드백 받기", key=f"{person_name}_reflection_feedback"):
        ...
        if text == "":
            ...
            if english_letters > korean_letters:
                ...
                if (
                    "never give up" in text.lower()
                    or "believe" in text.lower()
                    or "practice" in text.lower()
                ):
                    st.success("오늘 배운 핵심 표현을 잘 활용했습니다.")
                else:
                    st.info("believe in yourself, never give up, keep practicing 중 하나를 넣어 보면 더 좋습니다.")
...
""")
            ...
            st.markdown("---")
            st.success(
                "좋습니다. 이 활동의 핵심은 정답을 맞히는 것이 아니라, "
                "지문 속 인물의 태도를 내 삶과 연결해 보는 것입니다."
            )
