...
st.markdown(
    "<div class='sub-title'>이모지와 한국어 뜻을 보고, 영어 발음을 듣고, 직접 따라 말하는 자기진단 미션입니다.</div>",
    unsafe_allow_html=True
)
...
# 발음 버튼
# =========================
def audio_button(label, text, height=56):
    ...
    components.html(
        f"""
        <div style="font-family: Arial, sans-serif; display:flex; align-items:center; justify-content:center; gap:12px; height:48px;">
...
if not st.session_state.mission_started:
    ...
    st.write("총 50개의 단어가 랜덤으로 나옵니다. 듣고 따라 말한 뒤 자기진단을 해 보세요.")

    if st.button("🗣️ 50개 말하기 미션 시작하기"):
        start_new_mission()
        st.rerun()
